###############################################################################
# File: geneateStructuralComparisonsSpreadsheet.py
# Author: Carlos Bobed
# Date: March 2018
# Comments: script to import the data of the codified transactions into a
#       sqlite database
# Modifications:
#       June 2021: Generalization to make it more useful, now it can
#       work with any set of results given in .csv
#       - the csv with the measures is obtained with the scripts
#       measures*-*.sh, which show the entry point to calculate the oriented measure
#       - the csv with the ratios is obtained with the script
#       calculateRatios.sh (modified as required), which takes all the reports
#       obtained by KRIMP/SLIM and builds a ratio .csv
###############################################################################
...
if __name__ == "__main__":
    print (sys.argv)
    if (len(sys.argv) != 5):
        print ("uso: python geneateStructuralComparisonsSpreadsheet.py inputCSVFilenameRatios inputCSVMeasureRatios outFile prefixCT")
        sys.exit()
    # we now accept the directory of the gathered outputs as parameter
    FileStrings.prefixCT = sys.argv[4]+"/"
