...
public class KrimpSlimAlgorithm extends AbstractKrimpSlimAlgorithm {
	 ...
	 * @return
	 */
	protected KItemset generateCandidate(final CodeTable refCode, double standardSize, int minSup , final HashSet<KItemset> testedCandidates) {
				...
				if(! this._topKCandidates.isEmpty()) {
					...
					while(itTopKCandidates.hasNext()) {
						...
						if(! testedCandidates.contains(candidate)) {
							if(codetable.estimateUsageCombination(tmpX, tmpY) > 0 && codetable._index.getCodeSupport(candidate) >= minSup) {
								return candidate;
							}
						}
					}
...
//										logger.debug("testing new candidate with support " + codetable._index.getCodeSupport(candidatePotential));
										if(candidatePotential.size() <= codetable._index.getMaxSize()
												&& codetable._index.getCodeSupport(candidatePotential) >= minSup
