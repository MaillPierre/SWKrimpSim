///////////////////////////////////////////////////////////////////////////////
//File: UpdateFilesTestCanonizedChangesets.java 
//Author: Carlos Bobed
//Date: February 2018
...
public class UpdateFilesTestCanonizedChangesets {

private static Logger logger = Logger.getLogger(UpdateFilesTestCanonizedChangesets.class);
	...
	try  {
		...
		if (!filenames.isEmpty()) {
			...
			filenames.stream().forEach( fileID -> { 
				...
				ChangesetFile changeFileCanonized = new ChangesetFile(year, month, day, hour, number, 
											fileID+ChangesetFile.ADDED_CANONIZED_EXTENSION, 
											fileID+ChangesetFile.DELETED_CANONIZED_EXTENSION);
				
				Changeset changesetCanonized = new Changeset(changeFileCanonized, false); 
				
				if (changeset.getAddTriples().isEmpty() && changesetCanonized.getAddTriples().isEmpty() ) { 
					System.out.println(fileID+ChangesetFile.ADDED_EXTENSION + " both empty"); 
				}
				else { 
					if (changeset.getAddTriples().isEmpty() || changesetCanonized.getAddTriples().isEmpty() ) { 
						...
						StmtIterator stmtIt = changesetCanonized.getAddTriples().listStatements(); 
						boolean addOK = true; 
						...
						if (!addOK) { 
							...
							System.out.println(fileID+ChangesetFile.ADDED_CANONIZED_EXTENSION+" Size: "+changesetCanonized.getAddTriples().size()); 
						}
					}
				}
				
				if (changeset.getDelTriples().isEmpty() && changesetCanonized.getDelTriples().isEmpty() ) { 
					System.out.println(fileID+ChangesetFile.DELETED_EXTENSION + " both empty"); 
				}
				else { 
					if (changeset.getDelTriples().isEmpty() || changesetCanonized.getDelTriples().isEmpty() ) { 
						...
						// we have to compare them
						StmtIterator stmtIt = changesetCanonized.getDelTriples().listStatements(); 
						boolean addOK = true; 
						while (stmtIt.hasNext() && addOK) { 
							addOK = changeset.getDelTriples().contains(stmtIt.next()); 
						}
						if (!addOK) { 
							wrongFiles.add(fileID+ChangesetFile.DELETED_CANONIZED_EXTENSION); 
						}
							...
							System.out.println(fileID+ChangesetFile.DELETED_CANONIZED_EXTENSION+" Size: "+changesetCanonized.getDelTriples().size()); 
						}
					}
				}
