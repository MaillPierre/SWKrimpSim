///////////////////////////////////////////////////////////////////////////////
// File: CoverAnalysis.java 
// Author: Carlos Bobed
// Date: June 2017
// Comments: Adaptation of the KrimpAlgorithm main method to analyse the 
// 		different covers/codifications of the transactions given a codetable. 
...
//						}
				...
				try {
					...
					logger.debug("----- ORDERED SCandidateOrder ---- ");
					krimpCT.orderCodesStandardCandidateOrder();
					logger.debug(krimpCT);
					logger.debug("----- ORDERED SCoverageOrder ---- "); 
					krimpCT.orderCodesStandardCoverageOrder();
					logger.debug(krimpCT);
					

					logger.debug("-------- CODIFICATIONS --------"); 
					
					for (Itemset trans: realtransactions) {
						logger.debug("Transaction: "+trans); 
						logger.debug("IsCover coverage:");
						for (Itemset auxCode: krimpCT.codifyUsingIsCover(trans)) {
							logger.debug(auxCode);
						}
						logger.debug("Codify covereage:"); 
						for (Itemset auxCode: krimpCT.codify(trans)) {
							logger.debug(auxCode);
						}
