///////////////////////////////////////////////////////////////////////////////
//File: LUBMSyntheticAdder.java 
//Author: Carlos Bobed
//Date: July 2018
//Comments: Class that takes a two LUBM generated models (they must 
// 	have been converted from OWL to RDF - e.g., using rdf2rdf.jar), and generates 
// 	the add triples in the format required by all the dblpharvest package (as 
// 	in DBpediaLive). 
//Modifications:
///////////////////////////////////////////////////////////////////////////////
...
public class LUBMSyntheticAdder {

	private static Logger logger = Logger.getLogger(LUBMSyntheticAdder.class);
	...
	public static String ADDED_MODEL_OPTION = "addedModel"; 
	...
	public static void main(String[] args) {
		...
		options.addOption(ADDED_MODEL_OPTION, true, "filename with the model to be progressively added"); 
		...
		try  {
			...
			if(helpAsked) {
				...
				formatter.printHelp( "LUBMSyntheticAdder", options );
				System.exit(0);
			} 
			...
			long modelAddedTimeStart = System.nanoTime(); 
			Model addedModel = ModelFactory.createDefaultModel(); 
			RDFDataMgr.read(addedModel,  cmd.getOptionValue(ADDED_MODEL_OPTION));
			long addedModelLoadingTime = System.nanoTime() - modelAddedTimeStart; 
				...
				try { 
					...
					if (canonizing) { 
						changeFile = new ChangesetFile(year, month, day, hour, number, 
												currentUpdateFilename+ChangesetFile.ADDED_EXTENSION, 
												currentUpdateFilename+ChangesetFile.DELETED_EXTENSION); 
					} 
					else { 
						changeFile = new ChangesetFile(year, month, day, hour, number, 
